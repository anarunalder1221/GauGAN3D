...
% 12, 246
for f = 12%%:length(folder)
    label = imread(direct + "/" + folder{f} + ".png");
    label_nearest = imresize(label, [256 256], "nearest");
...
%     image = imread("/media/anaru/Seagate Expansion Drive/geoPose3K_SPADE/" + ...
%         "Images/" + folder{f} + ".jpg");
%     dist = imresize(dist, [256 256], 'nearest');
%     image = imresize(image, [256 256], 'nearest');
    ...
    for i = 1:length(label(:,1,1))
        for j = 1:length(label(1,:,1))
            ...
            if i ~= 1 && label(i,j,1) ~= label_nearest(i,j,1)
                dist(i,j) = dist(i-1,j);
            end
        end
    end

    %Creating the xyzc vectors for 3D plotting
    count = 0;
    ...
    for i = 1:length(dist(:,1))
        for j = 1:length(dist(1,:))
            if dist(i,j) ~= 256
                ...
                a = 1 * 10^-4;
                for k = dist(i,j):testLen
                    height = min(round(a*iter^3 + i), length(dist(:,1)));
                    if height == length(dist(:,1)) && iter > 0
                        break;
                    end
                    iter = iter + 1;
                    count = count + 1;
                    ...
                    y(count) = height;
                    z(count) = k;
                    c(count,1) = image(height,j,1);
                    c(count,2) = image(height,j,2);
                    c(count,3) = image(height,j,3);
                    check = check + 1;
                end
            end
        end
...
set(gca, 'Color', 'k')
set(gcf,'unit','norm','position',[0 0 0.6 0.6])
